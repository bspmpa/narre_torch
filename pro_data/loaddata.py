...
import pickle
import numpy as np
TPS_DIR = '/code/r9/xai/NARRE/data/movie_tv/'
...
def numerize(tp):
    item2id = dict((sid, i) for (i, sid) in enumerate(itemcount['item_id']))
    user2id = dict((uid, i) for (i, uid) in enumerate(usercount['user_id']))
