...
parser.add_argument("--valid_data", default = "/code/r9/xai/NARRE/data/boss/music_valid.csv", help = "Data for validation")
parser.add_argument("--test_data", default = "/code/r9/xai/NARRE/data/boss/music_test.csv", help = "Data parameters")
parser.add_argument("--train_data", default = "/code/r9/xai/NARRE/data/boss/music_train.csv", help = "Data for training")

parser.add_argument("--user_review", default = '/code/r9/xai/NARRE/data/boss/user_review', help = "User's reviews") # 300
parser.add_argument("--item_review", default = '/code/r9/xai/NARRE/data/boss/item_review', help = "Item's reviews")
parser.add_argument("--user_review_id", default = '/code/r9/xai/NARRE/data/boss/user_rid', help =  "Number of filters per filter size") # 100
parser.add_argument("--item_review_id", default = '/code/r9/xai/NARRE/data/boss/item_rid', help = "Dropout keep probability ")
parser.add_argument("--stopwords", default = "../data/stopwords")
...
#     u_text, i_text = build_input_data(u_text, i_text, vocabulary_user, vocabulary_item)
    ...
    return [u_text, i_text, y_train, y_valid, uid_train, iid_train, uid_valid, iid_valid, user_num, item_num, reid_user_train,
...
#                 s1 = s1.split(" ")
    ...
    padding_user = np.zeros((2, 6))
    padding_item = np.zeros((2, 5))
...
#     print('^^^^^^^^^^^')
#     print(np.array(list(u_rid.values())).shape)
    ...
    for line in f_valid:
        ...
        if int(line[0]) in u_text.keys():
            ...
            if len(u_rid[int(line[0])])!=2:
                print(u_rid[int(line[0])])
                
        else:
            u_text[int(line[0])] = padding_user
            u_rid[int(line[0])] = [int(0), int(0)]
            reid_user_valid.append(u_rid[int(line[0])])

        if int(line[1]) in i_text.keys():
            reid_item_valid.append(i_rid[int(line[1])])
        else:
            i_text[int(line[1])] = padding_item
            i_rid[int(line[1])] = [int(0), int(0)]
            reid_item_valid.append(i_rid[int(line[1])])

        y_valid.append(float(line[2]))
    
#     print('00000000000')
#     print(np.array(reid_user_valid).shape)
#     print(np.array(reid_item_valid).shape)
    ...
    review_len_u = np.array([len(j) for i in u_text.values() for j in i]) # 每一个user 评论的长度
...
#     user_num = len(u_text)
#     item_num = len(i_text)
    user_num = np.array(list(u_text.keys())).max()
    item_num = np.array(list(i_text.keys())).max()
...
#     FLAGS.flag_values_dict()
    ...
    u_text, i_text, y_train, y_valid, uid_train, iid_train, uid_valid, iid_valid, user_num, item_num, reid_user_train, reid_item_train, reid_user_valid, reid_item_valid = \
        load_data(args.train_data, args.valid_data, args.user_review, args.item_review, args.user_review_id,
                  args.item_review_id, args.stopwords)
    ...
    kk = [df[i].factorize()[0]+1 for i in user_ft+item_ft]
    uuu = [j+'_uniform' for j in user_ft+item_ft]
    ...
    para['review_num_u'] = len(list(u_text.values())[0])
    para['review_num_i'] = len(list(i_text.values())[0])
    para['review_len_u'] = len(list(u_text.values())[0][0])
    para['review_len_i'] = len(list(i_text.values())[0][0])
    para['user_vocab'] = num_users
    para['item_vocab'] = num_items
